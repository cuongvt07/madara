...
function madara_customize_register_manga_reading( $wp_customize ) {
    ...
    $arr_settings = array(
        ...
        'manga_reading_style' => 'list',
        'manga_reading_side2side' => 'off',
        'manga_chapters_select_order' => '',
        'manga_reading_content_gaps' => '',
        'manga_reading_images_per_page' => '1',
    ...
    $wp_customize->add_control(
        new Skyrocket_Toggle_Switch_Custom_control(
            $wp_customize,
            'manga_reading_side2side',
            array(
                'label'          => esc_html__( 'Side by Side', 'madara' ),
                'section'        => $section,
                'settings'       => 'manga_reading_side2side',
                'type'           => 'select',
                'description' => esc_html__( 'Display 2 images side by side', 'madara' ),
                'choices' => array(
                    'off' => esc_html__( 'Off', 'madara' ),
                    'on' => esc_html__( 'On', 'madara' )
				),
    ...
    for($i = 1; $i <= 10; $i++){
        $images_per_page_choices[$i] = esc_html__( $i . ' images', 'madara' );
    }
    ...
    $wp_customize->add_control(
        new WP_Customize_Control(
            ...
            array(
                ...
                'type'           => 'select',
                'description' => '',
                'choices' => $images_per_page_choices
            )
        )
    );
