jQuery(document).ready(function ($) {
	...
	const toggleReadingStyleControls = (val) => {
		if (val == 'paged') {
			...
			$('#customize-control-manga_reading_side2side').show();
			var is_side2side = wp.customize.settings.settings.manga_reading_side2side.value;
			if (is_side2side == 'off') {
			...
			$('#customize-control-manga_reading_side2side').hide();
			$('#customize-control-manga_reading_images_per_page').hide();
		}
	}
	...
	const toggleReadingSide2SideControls = (val) => {
		if (!val || val == 'on') {
			$('#customize-control-manga_reading_images_per_page').show();
		} else {
			$('#customize-control-manga_reading_images_per_page').hide();
	...
	wp.customize('manga_reading_side2side', function (control, a) {
		toggleReadingSide2SideControls(wp.customize.settings.settings.manga_reading_side2side.value);
		control.bind(function (value) {
			toggleReadingSide2SideControls(value);
		});
	});
