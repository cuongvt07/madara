...
function wp_manga_next_chapter_link_html($html, $next_link, $next_chap, $manga_id){
	...
	$cookie_ads_clicked = ($next_chap && isset($_COOKIE[$manga_id . '-' . $next_chap['chapter_id'] . '-ads-clicked'])) ? $_COOKIE[$manga_id . '-' . $next_chap['chapter_id'] . '-ads-clicked'] : '';

	if($cookie_ads_clicked == '1'){
		return $html;
	}

	$chapter_metas = ($next_chap && $next_chap['chapter_metas']) ? unserialize($next_chap['chapter_metas']) : array();
