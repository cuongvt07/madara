...
*/
	...
	$(document).on('click', 'form#loginform input[name="wp-submit"]', function (e) {
		...
		var mainUserSection = $('.c-sub-header-nav .c-modal_item');
		var offCanvasUserSection = $('.off-canvas .c-modal_item');
		...
		jQuery.ajax({
			...
			success: function (response) {
				if (response.success == true) {
					...
					if (mainUserSection.length != 0) {
						var mainLoginSection = mainUserSection.html();
					}

					if (offCanvasUserSection.length != 0) {
						var offCanvasLoginSection = offCanvasUserSection.html();
					}
					...
					jQuery.ajax({
						...
						beforeSend: function () {
							mainUserSection.html('<i class="fas fa-spinner fa-spin"></i>');
							offCanvasUserSection.html('<i class="fas fa-spinner fa-spin"></i>');
						},
						success: function (response) {
							if (response.success) {
								mainUserSection.empty();
								mainUserSection.append(response.data.main);

								offCanvasUserSection.empty();
								offCanvasUserSection.append(response.data.off_canvas);
							} else {
								if( typeof mainLoginSection !== 'undefined' ) {
									mainUserSection.html(mainLoginSection);
									offCanvasUserSection.html(mainLoginSection);
								}

								if( typeof offCanvasLoginSection !== 'undefined' ) {
									mainUserSection.html(offCanvasLoginSection);
									offCanvasUserSection.html(offCanvasLoginSection);
								}
							}

							$(document).trigger('manga-after-get-user-section', [response.data]);
						},
					});
